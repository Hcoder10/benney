...
families = []
with open('C:/Users/sarta/rosea/hotel_agents/data/families_part_multigenerational_bay_family__b0.jsonl') as f:
    for line in f:
        line = line.strip()
        if line:
...
def parse_hours(s):
    """Parse open-hours string -> list of (start_float, end_float) sessions."""
    if not s or s.strip().lower() in ('closed', '', '?'):
        return []
    s = s.strip()
    if re.match(r'(?i)^24h', s) or 'open 24 hours' in s.lower():
        return [(0.0, 24.0)]
    ...
    for sess in sessions:
        ...
        if m:
            ...
            if ap1 and ap1.upper() == 'PM' and h1 != 12:
                h1 += 12
            if ap1 and ap1.upper() == 'AM' and h1 == 12:
                h1 = 0
            if ap2 and ap2.upper() == 'PM' and h2 != 12:
                h2 += 12
            if ap2 and ap2.upper() == 'AM' and h2 == 12:
                h2 = 0
            start = h1 + m1 / 60.0
            end = h2 + m2 / 60.0
...
def hours_overlap(sessions, win_s, win_e):
    """True if ANY session overlaps (ANY-overlap) with the slot window."""
    for (s, e) in sessions:
...
for fam_rec in families:
    ...
    # Phase 1: slot-type and open-hours violations (also catches missing/placeholder)
    used_counts = {}
        ...
        if not tag_ok or not hrs_ok:
            ...
            if rep:
                ...
                if 'missing' in reasons:
                    total_missing += 1
                if 'tag' in reasons:
                    total_tag_violations += 1
                if 'hours' in reasons:
                    total_hours_violations += 1
...
print(f'  - Missing placeholders:    {total_missing}')
...
out_path = 'C:/Users/sarta/rosea/hotel_agents/data/families_clean_multigenerational_bay_family__b0.jsonl'
